...
def process_date_field(selected_text, full_text, start_idx, end_idx, date_formats, possible_separators, days, months, anchor_enabled, detect_separator, count_separators_before):
    pattern, extracted = get_date_pattern(selected_text, full_text, start_idx, possible_separators, date_formats, anchor_enabled, detect_separator, count_separators_before, days, months)
...
def get_date_pattern(selected_text, full_text, start_idx, possible_separators, date_formats, anchor_enabled, detect_separator, count_separators_before, days, months):
    cleaned_text = re.sub(r"(th|st|nd|rd)", "", selected_text).strip()
    ...
    extracted = match.group(1).strip() if match and match.groups() else cleaned_text
...
def get_date_format(date_str, possible_separators, days, months):
    ...
    for comp_type, _, _, length in components:
        if comp_type == "day":
            format_parts.append("EEE" if length == "short" else "EEEE")
        elif comp_type == "month":
